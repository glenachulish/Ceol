...
// ── FolkTuneFinder.com search + import ────────────────────────────────────────
const ftfSearchInput = document.getElementById("ftf-search-input");
const ftfSearchBtn   = document.getElementById("ftf-search-btn");
const ftfResults     = document.getElementById("ftf-results");
let _ftfTuneData     = null;  // currently previewed tune

async function runFtfSearch() {
  const q = ftfSearchInput.value.trim();
  if (!q) return;
  ftfResults.innerHTML = '<p class="loading" style="padding:1rem 0">Searching FolkTuneFinder…</p>';
  ftfSearchBtn.disabled = true;
  try {
    const params = new URLSearchParams({ q });
    const data = await apiFetch(`/api/folktunefinder/search?${params}`);
    if (!data.tunes || !data.tunes.length) {
      ftfResults.innerHTML = '<p class="empty" style="padding:1rem 0">No results found.</p>';
      return;
    }
    ftfResults.innerHTML = data.tunes.map(t => `
      <div class="session-result-row" data-ftf-id="${escHtml(String(t.id))}">
        <div class="session-result-info">
          <span class="session-result-name">${escHtml(t.name)}</span>
          ${t.rhythm ? `<span class="badge">${escHtml(t.rhythm)}</span>` : ""}
          ${t.key ? `<span class="session-result-meta">${escHtml(t.key)}</span>` : ""}
        </div>
        <button class="btn-primary ftf-preview-btn" data-ftf-id="${escHtml(String(t.id))}" data-ftf-name="${escHtml(t.name)}">
          Preview
        </button>
      </div>
    `).join("");

    ftfResults.querySelectorAll(".ftf-preview-btn").forEach(btn => {
      btn.addEventListener("click", async () => {
        btn.disabled = true;
        btn.textContent = "Loading…";
        try {
          const tuneData = await apiFetch(`/api/folktunefinder/tune/${encodeURIComponent(btn.dataset.ftfId)}`);
          showFtfPreview(tuneData);
        } catch (err) {
          btn.textContent = "Error";
          setTimeout(() => { btn.textContent = "Preview"; btn.disabled = false; }, 2000);
        }
      });
    });
  } catch (err) {
    ftfResults.innerHTML = '<p class="empty" style="padding:1rem 0">Could not reach FolkTuneFinder. Check your connection.</p>';
    console.error(err);
  } finally {
    ftfSearchBtn.disabled = false;
  }
}

function showFtfPreview(tuneData) {
  _ftfTuneData = tuneData;
  document.getElementById("ftf-search-pane").classList.add("hidden");
  const preview = document.getElementById("ftf-preview");
  preview.classList.remove("hidden");
  ...
  saveBtn.style.background = "";
  saveBtn.style.opacity = "";
  const status = document.getElementById("ftf-save-status");
  status.textContent = "";
  status.className = "notes-status";

  // Render sheet music
  const renderEl = document.getElementById("ftf-sheet-render");
  renderEl.innerHTML = "";
  if (tuneData.abc && typeof ABCJS !== "undefined") {
    ABCJS.renderAbc("ftf-sheet-render", expandAbcRepeats(tuneData.abc), {
      responsive: "resize",
      wrap: { preferredMeasuresPerLine: 4 },
      add_classes: true,
      staffwidth: 700,
    ...
    renderEl.innerHTML = '<p class="empty">No ABC notation available for this tune.</p>';
  }

  // ABC text panel
  const abcTextEl = document.getElementById("ftf-abc-text");
  if (abcTextEl) abcTextEl.textContent = tuneData.abc || "(no ABC)";
}

if (ftfSearchBtn) {
  ftfSearchBtn.addEventListener("click", runFtfSearch);
}
if (ftfSearchInput) {
  ftfSearchInput.addEventListener("keydown", e => { if (e.key === "Enter") runFtfSearch(); });
}

// Back button
const ftfBackBtn = document.getElementById("ftf-preview-back");
if (ftfBackBtn) {
  ftfBackBtn.addEventListener("click", () => {
    _ftfTuneData = null;
    document.getElementById("ftf-preview").classList.add("hidden");
    document.getElementById("ftf-search-pane").classList.remove("hidden");
  });
}

// Save button
const ftfSaveBtn = document.getElementById("ftf-save-btn");
if (ftfSaveBtn) {
  ftfSaveBtn.addEventListener("click", async () => {
    if (!_ftfTuneData) return;
    const btn = ftfSaveBtn;
    const status = document.getElementById("ftf-save-status");
    btn.disabled = true;
    btn.textContent = "Saving…";
    try {
      const res = await fetch("/api/folktunefinder/import", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          tune_id: _ftfTuneData.id,
          title: _ftfTuneData.title,
        }),
      });
      const data = await res.json();
      if (res.ok) {
