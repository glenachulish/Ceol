...
// setHandler — safely attach a single event listener to a persistent DOM element.
// Clones the element (stripping existing listeners) before attaching the new one.
// Use instead of getElementById(id).addEventListener() on elements that persist
// across multiple render calls (modals, overlays, fixed buttons) to prevent
// listener accumulation and double-fire bugs.
function setHandler(id, event, fn) {
...
// ─── Alternate-bar shading (added cluster B patch 7, 12 May 2026) ────
// Call AFTER every ABCJS.renderAbc(containerId, …) to add a tinted
// background <rect> behind every other measure. Idempotent.
function _shadeBars(containerId) {
  // Allow either an ID or an element
  const root = typeof containerId === "string"
    ? document.getElementById(containerId)
    : containerId;
  if (!root) return;
  // abcjs marks each staff line as .abcjs-l<N>, each measure within
  // as .abcjs-m<N>. We pick up every measure group regardless of staff.
  const measures = root.querySelectorAll("g[class*='abcjs-m']");
  if (!measures.length) return;
  // Sort by their absolute X position so the alternation reads
  // left-to-right within each line. abcjs already gives them in
  // document order which matches reading order, so iterating directly
  // is fine.
  let idx = 0;
  measures.forEach(g => {
    // Skip if already shaded
    if (g.querySelector(":scope > rect.abcjs-bar-shade")) {
      idx++;
      return;
    }
    let bb;
    try { bb = g.getBBox(); } catch { return; }
    if (!bb || !bb.width || !bb.height) { idx++; return; }
    ...
    rect.setAttribute("class",
      "abcjs-bar-shade " + (idx % 2 === 0 ? "bar-shade-even" : "bar-shade-odd"));
    // Insert as the FIRST child so it sits behind everything
    g.insertBefore(rect, g.firstChild);
    idx++;
  });
}
// Convenience: shade every container under document with rendered abc
function _shadeAllRenderedBars() {
...
// ─── Auto-shade observer (cluster B patch 7) ──────────────────────────
// Whenever a render container gets a new SVG, shade its bars.
(function _initBarShadeObserver() {
  function _maybeShade(node) {
    if (!(node instanceof Element)) return;
    // Walk up to find a render container ancestor (or self)
    let el = node;
    while (el && el !== document.body) {
      if (el.id && (el.id.endsWith("-render") || el.id.includes("-render-"))) {
        // Wait one frame so abcjs finishes any DOM updates
        requestAnimationFrame(() => _shadeBars(el));
        return;
      }
      el = el.parentElement;
    }
  ...
  const obs = new MutationObserver(muts => {
    for (const m of muts) {
      m.addedNodes.forEach(_maybeShade);
    }
  });
  // Wait until DOM is ready, then observe everything
  function _start() {
    obs.observe(document.body, { childList: true, subtree: true });
    // Catch anything already rendered at startup
    _shadeAllRenderedBars();
  }
...
let _ceolMoreLastFire = 0;
window._ceolMoreToggle = function(e) {
  if (e) { e.stopPropagation(); }
  const now = Date.now();
  if (now - _ceolMoreLastFire < 300) return; // drop double-fire (iOS PWA touchend+click)
  _ceolMoreLastFire = now;
        ...
        .then(() => {
          ...
          if (pracSettings) {
            ...
            cursorControl.onFinished = function() {
              ...
              // BUG 4 fix: apply setWarp AFTER play() resolves (race condition)
              const applyWarp = () => {
                try { _abcFsSynthCtrl.setWarp(curWarp); } catch(_e) {}
                try {
                  const warpEl = document.querySelector("#abc-fs-audio .abcjs-midi-warp input");
                  if (warpEl) {
                    warpEl.value = curWarp;
                    warpEl.dispatchEvent(new Event("input",  { bubbles: true }));
                    warpEl.dispatchEvent(new Event("change", { bubbles: true }));
                  }
                } catch(_e) {}
              };
              try {
                const playResult = _abcFsSynthCtrl.play();
                if (playResult && typeof playResult.then === 'function') {
                  playResult.then(applyWarp).catch(() => applyWarp());
                } else {
                  applyWarp();
                }
              } catch(_e) {}
...
// BUG 1 fix: wire BOTH the quick (#quick-*) and filter-panel (#filter-*) buttons.
// The prominent #filter-hitlist-btn / #filter-favourite-btn used to be dead UI.
const _allHitlistBtns   = document.querySelectorAll("#quick-hitlist-btn, #filter-hitlist-btn");
const _allFavouriteBtns = document.querySelectorAll("#quick-fav-btn, #filter-favourite-btn");

_allHitlistBtns.forEach(btn => btn.addEventListener("click", () => {
  state.hitlist = !state.hitlist;
  _allHitlistBtns.forEach(b => b.classList.toggle("active", state.hitlist));
  state.page = 1;
  loadTunes();
}));

_allFavouriteBtns.forEach(btn => btn.addEventListener("click", () => {
  state.favourite = !state.favourite;
  _allFavouriteBtns.forEach(b => b.classList.toggle("active", state.favourite));
  state.page = 1;
  loadTunes();
}));
...
clearBtn.addEventListener("click", () => {
  ...
  // BUG 1 fix: clear BOTH quick + filter-panel buttons
  _allHitlistBtns.forEach(b => b.classList.remove("active"));
  _allFavouriteBtns.forEach(b => b.classList.remove("active"));
...
function _renderPracticeMusic(pracAbc) {
  ...
  try {
    ...
    const cursorControl = {
      ...
      onFinished() {
        // Clear note highlights from previous loop
        try {
          document.querySelectorAll("#prac-sheet-render .abcjs-highlight")
            .forEach(el => el.classList.remove("abcjs-highlight"));
        } catch(_) {}
        ...
        // BUG 2 fix: use _pracSettings (was incorrectly using _pracFinalWarp etc.)
        try {
        ...
        // Loop: restart, then apply new warp AFTER play() resolves
        // (setWarp before play() was being ignored — race condition)
        if (_pracSynthCtrl) {
          try {
            const applyWarp = () => {
              try { _pracSynthCtrl.setWarp(_pracCurWarp); } catch(_) {}
              // Also nudge the warp slider so abcjs picks up the value
              try {
                const warpEl = document.querySelector(
                  '#prac-player-container .abcjs-midi-warp input'
                );
                if (warpEl) {
                  warpEl.value = _pracCurWarp;
                  warpEl.dispatchEvent(new Event('input',  { bubbles: true }));
                  warpEl.dispatchEvent(new Event('change', { bubbles: true }));
                }
              } catch(_) {}
            };
            const playResult = _pracSynthCtrl.play();
            if (playResult && typeof playResult.then === 'function') {
              playResult.then(applyWarp).catch(() => applyWarp());
            } else {
              applyWarp();
...
;(function _ceolAudioProminence() {
  /* ceol-audio-prominence 5 May 2026 */
  // After the tune modal opens and sheet music renders, check whether
  // the sheet-music-render div is empty. If so (audio-only tune):
  //  1. Show a "No sheet music yet" placeholder.
  //  2. Auto-trigger the first Play MP3 button so the audio player
  //     is ready at the top of the page without needing to scroll.

  function _promoteAudioIfNoAbc() {
    const render = document.getElementById('sheet-music-render');
    if (!render) return;

    const hasAbc = render.querySelector('svg, canvas, .abcjs-container');
    if (hasAbc) {
      // Has sheet music — remove placeholder if we added one previously
      const ph = document.getElementById('_ceol-no-abc-placeholder');
      if (ph) ph.remove();
      return;
    }
    ...
    // No ABC content
    // 1. Placeholder
    if (!document.getElementById('_ceol-no-abc-placeholder')) {
      const ph = document.createElement('div');
      ph.id = '_ceol-no-abc-placeholder';
      ph.style.cssText = [
        'padding: 1.5rem 1rem',
      ...
      ph.textContent = 'No sheet music yet';
      render.parentElement.insertBefore(ph, render.nextSibling);
    }

    // 2. Auto-load first MP3 into the inline player (only if not already loaded)
    const audioEl = document.getElementById('inline-mp3-player');
    if (!audioEl) return;
    if (audioEl.src && !audioEl.classList.contains('hidden')) return; // already loaded

    const firstPlayBtn = document.querySelector('.media-play-btn[data-url]');
    if (!firstPlayBtn) return;

    // Simulate click to load + show
    firstPlayBtn.click();

    // Scroll the audio element into view smoothly
    setTimeout(() => {
      audioEl.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    }, 80);
  }

  // Hook: watch for the tune modal becoming visible, then run after render settles
  const _observer = new MutationObserver(function(mutations) {
    for (const mut of mutations) {
      for (const node of mut.addedNodes) {
        if (node.nodeType !== 1) continue;
        // Modal opened: wait for abcjs to render (or not), then check
        if (node.id === 'sheet-music-render' || node.closest?.('#tab-music')) {
          setTimeout(_promoteAudioIfNoAbc, 300);
          return;
        }
      }
      // Also catch attribute changes on tab panels (tab switch)
      if (mut.type === 'attributes' && mut.target.id === 'tab-music') {
        setTimeout(_promoteAudioIfNoAbc, 300);
      }
    }
  });

  // Also watch for tune rendering completing (abcjs fires a custom event or
  // we can watch for SVG appearing/disappearing in sheet-music-render).
  // Simplest: watch the render div itself for child list changes.
  function _attachRenderObserver() {
    const render = document.getElementById('sheet-music-render');
    if (!render) return;
    const ro = new MutationObserver(() => setTimeout(_promoteAudioIfNoAbc, 200));
    ro.observe(render, { childList: true, subtree: false });
  }

  // Watch the whole document for the modal to appear, then attach the render observer
  const _docObserver = new MutationObserver(function(mutations) {
    for (const mut of mutations) {
      for (const node of mut.addedNodes) {
        if (node.nodeType !== 1) continue;
        if (node.id === 'tab-music' || node.querySelector?.('#sheet-music-render')) {
          _attachRenderObserver();
          setTimeout(_promoteAudioIfNoAbc, 400);
        }
      }
    }
  });
  ...
  _attachRenderObserver();
  setTimeout(_promoteAudioIfNoAbc, 500);

  // Re-run on tab switch (clicking Sheet Music tab on an already-open tune)
  document.addEventListener('click', function(e) {
    const tabBtn = e.target.closest?.('[data-tab="music"], [href="#tab-music"], .tab-btn');
    if (tabBtn) setTimeout(_promoteAudioIfNoAbc, 350);
  }, true);
})();
...
;(function _ceolAudioReliability() {
  /* ceol-audio-reliability 5 May 2026 */
  // --- AUDIO CONTEXT RESUME HOOKS ---
  // iOS suspends Web Audio when the page is hidden / screen locked.
  // Resume on every plausible "user is back" event.
  ...
  function _findCtxs() {
    ...
    var candidates = [
      ...
      window._synthController && window._synthController._audioContext,
      window._synthController && window._synthController.audioContext,
      window._synthController && window._synthController.midiBuffer && window._synthController.midiBuffer.audioContext
    ];
    for (var i = 0; i < candidates.length; i++) {
      var c = candidates[i];
      if (c && typeof c.resume === 'function' && !seen.has(c)) {
        seen.add(c); ctxs.push(c);
      }
    }
    return ctxs;
  }
  ...
  var _lastResume = 0;
  function _resumeAll(reason) {
    var now = Date.now();
    if (now - _lastResume < 50) return; // throttle
    _lastResume = now;
    var ctxs = _findCtxs();
      ...
      if (ctx.state === 'suspended' || ctx.state === 'interrupted') {
        ...
        try {
          ctx.resume().then(function() {
            _dbg('resume:' + prevState + '->' + ctx.state + ' (' + reason + ')');
          }).catch(function(e) {
            _dbg('resume FAIL: ' + e.message + ' (' + reason + ')');
          });
        } catch (e) {
          _dbg('resume threw: ' + e.message);
        }
      }
    }
  }
  ...
  document.addEventListener('visibilitychange', function() {
    if (document.visibilityState === 'visible') {
      // Two attempts: one immediate, one after a short delay
      // (iOS sometimes won't accept resume() until layout settles)
      setTimeout(function() { _resumeAll('visibility'); }, 50);
      setTimeout(function() { _resumeAll('visibility-late'); }, 400);
    } else {
      _dbg('hidden');
    }
  });
  window.addEventListener('focus',    function() { _resumeAll('focus'); });
  window.addEventListener('pageshow', function() { _resumeAll('pageshow'); });

  // Capture-phase click — broadest possible user-gesture trigger.
  // Cheap (no-op when context already running).
  document.addEventListener('click', function(e) {
    _resumeAll('click');
  }, true);
  // Touch as well — some iOS gestures fire only touchend
  ...
  var _debugOn = /[?&]debug=1\b/.test(location.search);
  var _overlay = null;
  var _logLines = [];
  ...
  function _dbg(msg) {
    ...
    _logLines.push(line);
    if (_logLines.length > 12) _logLines.shift();
    _renderOverlay();
  }
  ...
  function _renderOverlay() {
    ...
    if (!_overlay) {
      ...
      _overlay.id = '_ceol-audio-debug';
      _overlay.style.cssText = [
        'position:fixed','bottom:0','left:0','right:0','z-index:99999',
        'background:rgba(0,0,0,0.85)','color:#0f0',
        'font:11px/1.3 monospace','padding:6px 8px','max-height:180px',
        'overflow-y:auto','pointer-events:auto',
      ].join(';');
      _overlay.addEventListener('click', function() {
        _logLines = []; _renderOverlay();
      });
      document.body.appendChild(_overlay);
    }
    var ctxs = _findCtxs();
    var head = 'AUDIO DEBUG ' + (ctxs.length ? ctxs.map(function(c){return c.state;}).join('|') : 'no ctx') +
               ' [tap to clear]';
    _overlay.innerHTML = '<div style="color:#ff0">' + head + '</div>' +
                         _logLines.map(function(l){return '<div>'+l.replace(/[<>]/g,'')+'</div>';}).join('');
  }

  if (_debugOn) {
    // Initialise overlay
    if (document.body) _renderOverlay();
    else document.addEventListener('DOMContentLoaded', _renderOverlay);
    // Periodic state check
    setInterval(function() { _dbg('state=' + (_findCtxs().map(function(c){return c.state;}).join(',') || 'none')); }, 3000);
  }

  // Expose for manual checks via URL bar JS or the debug shortcut
  window._ceolAudioStatus = function() {
    return _findCtxs().map(function(c) { return { state: c.state, ct: c.currentTime, sr: c.sampleRate }; });
  };
